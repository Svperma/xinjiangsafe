...
public class PaymentForOnlineService {
	...
	private static String queryRefundReqURL = Configuration.getInstance()
			.getValue("queryRefundReqURL"); // 请求地址
	...
	/**
	 * 订单查询请求参数 该方法是根据《易宝支付产品通用接口（HTML版）文档 v3.0》怎样查询订单进行的封装
	 * 具体参数含义请仔细阅读《易宝支付产品通用接口（HTML版）文档 v3.0》 商户订单号
	 * 
	 * @param p2_Order
	 * @return queryResult
	 */
	public static QueryResult queryByOrder(String p2_Order) {
		...
		newHmac = DigestUtil.getHmac(
				new String[] { qr.getR0_Cmd(), qr.getR1_Code(),
						qr.getR2_TrxId(), qr.getR3_Amt(), qr.getR4_Cur(),
						qr.getR5_Pid(), qr.getR6_Order(), qr.getR8_MP(),
						qr.getRb_PayStatus(), qr.getRc_RefundCount(),
