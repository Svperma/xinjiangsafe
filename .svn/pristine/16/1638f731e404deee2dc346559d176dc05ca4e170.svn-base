...
<html>
  ...
  <input type="hidden" name="riskCode" id="riskCode" value="${policyMain.riskCode }"/>
  <input type="hidden" name="insureCode" id="insureCode" value="${policyMain.insurerCode }"/>
  <input type="hidden" name="areaCode" id="areaCode" value="${policyManager.areaCode }"/>
  <input type="hidden" name="businessNO" id="businessNO" value="${policyMain.businessNo }" />
  	...
  	<div class="tab-top-div">
		...
		<table class="nav-tab">
			...
			<tr>
				...
				<td colspan="7">${alInsured.insuredname}</td>
			</tr>
			<tr>
				<td colspan="2">联系地址</td>
				<td colspan="2">${alInsured.insuredaddress}</td>
				<td colspan="2">邮箱</td>
				<td colspan="2">${alInsured.email}</td>
			</tr>
			<tr>
				<td colspan="2">联系电话</td>
				<td colspan="2">${alInsured.phonenumber }</td>
				<td colspan="2">联系人</td>
				<td colspan="2">${alInsured.linkname }</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2">${inInsured.insuredname }</td>
				<td colspan="2">邮箱</td>
				<td colspan="2">${inInsured.email }</td>
			</tr>
			<tr>
				<td colspan="2">联系电话</td>
				<td colspan="2">${inInsured.phonenumber }</td>
				<td colspan="2">联系人</td>
				<td colspan="2">${inInsured.linkname }</td>
			</tr>
			<tr>
				<td colspan="1">联系地址</td>
				<td colspan="7">${inInsured.insuredaddress }</td>
			</tr>
			<tr>
				<td colspan="2">行业类别</td>
				<td colspan="2"><code:name  codeCode="${inInsured.remark }" codeType="IndustryCategories" /></td>
				<td colspan="2">经营范围</td>
				<td colspan="2"><code:name  codeCode="${inInsured.flag }" codeType="IndustryType" /></td>
			</tr>
			<tr>
				<td colspan="2">统一社会信用代码</td>
				<td colspan="2">
					${inInsured.identitynumber }
			...
			<tr>
				...
				<td colspan="2">
					<input readonly="readonly" id="pepSum" name="pepSum" type="text" value="${fn:length(employeeList)}" style="width:50%;">
                    <div class="maincont_mid_cont_right_bot" style="display:inline-block;">
				...
				<td colspan="2"><code:name  codeCode="${inInsured.policyno}" codeType="StandardLevel" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="4">${itemKind0901001.unitamount}元</td>
			</tr>
			<tr>
				<td colspan="3">每次事故赔偿限额</td>
				<td colspan="4"><fmt:formatNumber value="${itemKind0901001.amount}" pattern="##.##"></fmt:formatNumber>元</td>
			</tr>
			<tr>
			...
			<tr>
				<td>保险费  ￥：</td>
				<td colspan="7"><fmt:formatNumber value="${itemKind0901001.spreadsheetpremium}" pattern="##.##"></fmt:formatNumber>元</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					每次事故及累计赔偿限额${itemKind0901002.amount/10000}万元，其中：每次事故每人人身伤亡赔偿限额与主险一致；
每次事故每人医疗费用赔偿限额为每人人身伤亡赔偿限额的30%；每次事故财产损失为每次事故及累计赔偿限额的50%；
				</td>
				<td colspan="1">${itemKind0901002 eq null ? '未投保' : itemKind0901002.spreadsheetpremium}</td>
...
财产损失免赔额为人民币1000元</td>
			...
			<tr>
				...
				<td colspan="1">${itemKind0901003 eq null ? '未投保' : itemKind0901003.spreadsheetpremium}</td>
			...
			<tr>
				...
				<td colspan="1">${itemKind0901004 eq null ? '未投保' : itemKind0901004.spreadsheetpremium}</td>
			...
			<tr>
				...
				<td colspan="1">${itemKind0901005 eq null ? '未投保' : itemKind0901005.spreadsheetpremium}</td>
				<td colspan="3">免赔额为人民币500元。</td>
			</tr>
			<tr>
				<td colspan="4">人民币（大写）:${spreadsheetPremium}</td>
				<td colspan="4">总保险费:${policyMain.spreadsheetPremium}元</td>
			</tr>
			<tr>
				<td colspan="2">保险期间</td>
				<td colspan="6">自<fmt:formatDate type="date" value="${policyMain.startDate}" /> 日00:00:00时起 至<fmt:formatDate type="date" value="${policyMain.endDate}" />日23:59:59 止(以最终签发的保险单为准)为保证保险合同在拟定的保险起期生效，请您在保险起期前5个工作日前办理投保手续、提交投保资料并支付保费。如您未能按时办理投保手续或保费未能按时到账，导致保险合同未在拟定的保险起期生效，发生保险事故，本公司不承担保险责任。</td>
			</tr>
			<tr>
		...
		<div style="text-align:left;">
        	争议解决方式：<select id="toTalk" name="toTalk" style="width:150px;" >
                       <option value="${policyMain.argueSolution}">${policyMain.argueSolution == 1 ? '诉讼' : '仲裁' }</option>
<!--                        <option <c:if test="${policyMain.argueSolution==2 }">selected</c:if> value="2">仲裁</option> -->
          	        </select>
                    <c:if test="${policyMain.argueSolution==2 }">
                    	<input id="inp" name="toTalkName" class="select_list_inputa" type="text" maxlength="20" value="${policyMain.arbitBoardName}" readonly="readonly" style="float:none;height: 30px;">
    				</c:if>
        </div>   
	...
	<script type="text/javascript">
		function goZus() {
			fom.action = "${basePath}/insurePolicy/resubmitPolicy";
