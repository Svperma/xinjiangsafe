  	...
  	<div class="tab-top-div">
		...
		<table class="nav-tab">
			...
			<tr>
				...
				<td colspan="7">${policyManager.alName}</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2">${policyManager.inName }</td>
				<td colspan="2">邮箱</td>
			...
			<tr>
				...
				<td colspan="7">${policyManager.inAddress }</td>
			</tr>
			<tr>
				<td colspan="2">行业类别</td>
				<td colspan="2"><code:name  codeCode="${policyManager.classCode }" codeType="IndustryCategories" /></td>
				<td colspan="2">经营范围</td>
				<td colspan="2"><code:name  codeCode="${policyManager.businessClass }" codeType="IndustryType" /></td>
			</tr>
			<tr>
				<td colspan="2">统一社会信用代码</td>
				<td colspan="2">
					${policyManager.inBusinessLicenseNo }
				...
				<td colspan="2">
					${policyManager.inSafetyLicenseNo }
					<div class="maincont_mid_cont_right_bot" style="display:inline-block;">
			...
			<tr>
				...
				<td colspan="2"><code:name  codeCode="${policyManager.standardLevel}" codeType="StandardLevel" /></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="4"><fmt:formatNumber value="${policyManager.sumAmount}" pattern="##.##"></fmt:formatNumber>元</td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="3">
					每次事故及累计赔偿限额${sanamount}万元，其中：每次事故每人人身伤亡赔偿限额与主险一致；
...
财产损失免赔额为人民币1000元</td>
			...
			<tr>
				...
				<td colspan="6">自${startDate} 日00:00:00时起 至${endDate} 日23:59:59 止(以最终签发的保险单为准)为保证保险合同在拟定的保险起期生效，请您在保险起期前5个工作日前办理投保手续、提交投保资料并支付保费。如您未能按时办理投保手续或保费未能按时到账，导致保险合同未在拟定的保险起期生效，发生保险事故，本公司不承担保险责任。</td>
			</tr>
			<tr>
		...
		<div style="text-align:left;">
        	争议解决方式：<select id="toTalk" name="toTalk" style="width:150px;" >
                       <option value="${policyManager.toTalk}">${policyManager.toTalk == 1 ? '诉讼' : '仲裁' }</option>
