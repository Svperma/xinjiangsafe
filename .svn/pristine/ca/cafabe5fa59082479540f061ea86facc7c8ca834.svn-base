	...
	<div class="maincont">
		<div class="maincont_mid_cont_right">
			<div class="maincont_mid_cont_right_main">
				<form action="${basePath }/policyinfo/mycheckinitjgPain" id="fm"
					method="post">
...
<!-- 							<td align="center">业务员</td> -->
						...
						<c:forEach items="${sessionScope.pagination.resultList }"
							...
							<td class="employee" align="center">${mycheck.CURRENCY }</td>
							...
							<td align="center"><fmt:formatDate value="${mycheck.ENDDATE }"
									pattern="yyyy-MM-dd" /></td>
							<td class="sumAmount" align="center">${mycheck.SUMAMOUNT/10000 }</td>
							<td class="premium" align="center">${mycheck.SPREADSHEETPREMIUM }</td>
...
<!-- 							</td> -->
							...
							<td align="center">
								<c:if test="${mycheck.FLAG == '1' }">
									...
									href="/policyinfo/mypolicyshowP?businessno=${mycheck.BUSINESSNO }" target="mainFrame">查看</a>
									</td>
								</c:if>
						...
						<tr style="background-color:#00a6e3;">
							<td align="center" style="border:1px solid #ccc;color:#fff" >当前页面合计:</td>
							<td id="sumEmployee" align="center" style="border:1px solid #ccc;color:#fff" ></td>
							<td align="right" style="border:1px solid #ccc; border-right-style: 0px" colspan="3"></td>
							<td id="sumAmount1" align="center" style="border:1px solid #ccc;color:#fff" ></td>
							<td id="sumPrice" align="center" style="border:1px solid #ccc; border-left-style: 0px" colspan=""></td>
							<td align="left" style="border:1px solid #ccc;" colspan="5"></td>
						</tr>
						<tr style="background-color:#00a6e3;">
							<td align="center" style="border:1px solid #ccc;color:#fff" >当前条件合计:</td>
							<td align="center" style="border:1px solid #ccc;color:#fff" >${summation.CURRENCY}</td>
							<td align="right" style="border:1px solid #ccc; border-right-style: 0px" colspan="3"></td>
							<td id="" align="center" style="border:1px solid #ccc;color:#fff" >${summation.SUMAMOUNT/10000}</td>
							<td id="" align="center" style="border:1px solid #ccc; border-left-style: 0px" colspan="">${summation.SPREADSHEETPREMIUM}</td>
							<td align="left" style="border:1px solid #ccc;" colspan="5"></td>
						</tr>
					</table>
...
<script>
	...
	//计算当前页面
		window.onload = function () {
			//当前页面总保费计算
			var sumPrice = 0.00;
			var premiums = document.getElementsByClassName("premium");
			for(var i = 0 ; i < premiums.length ; i++ ){
				var p = parseFloat(premiums[i].innerHTML);
				if(isNaN(p)) {p = 0;}
				sumPrice += p ;
			}
			...
			//当前页面总限额计算
			var amount = 0.00;
			var sumAmounts = document.getElementsByClassName("sumAmount");
			for(var i = 0 ; i < sumAmounts.length ; i++ ){
				var a = parseFloat(sumAmounts[i].innerHTML);
				if(isNaN(a)) {a = 0;}
				amount += a;
			}
			document.getElementById("sumAmount1").innerHTML = amount;
			
			//当前页面总人数
			var sumEmployee = 0;
			var employees = document.getElementsByClassName("employee");
			for(var i = 0 ; i < employees.length ; i++ ){
				var p = parseFloat(employees[i].innerHTML);
				if(isNaN(p)) {p = 0;}
				sumEmployee += p;
			}
			document.getElementById("sumEmployee").innerHTML = sumEmployee;
		};
</script>
</html>
