...
<html>
 <body onload="changeSumAmount();" style="height:100%;background:#fff;">
  <form action="${basePath}/insurePolicy/getPlaceValue" enctype="multipart/form-data" method="post" name="fom">
		...
		<input type="hidden" name="insureCode" 	id="insureCode" 	value="${insureCode }" />
		<input type="hidden" name="areaCode" 	id="areaCode" 		value="${areaCode }" /> 
		<input type="hidden" name="businessNOee" id="businessNOee" 	value="${businessNo }" /> 
...
<!-- 			<p class="title-p">审核意见:${sessionScope.resultMap.UNDERDIRECTIONS }</p> -->
		...
		<table class="nav-tab">
			...
			<tr>
				...
				<td colspan="7"><input id="alName" name="alName" type="text" value="${alPep.insuredname }"></td>
			</tr>
			<tr>
				<td colspan="2">联系地址</td>
				<td colspan="2"><input id="alAddress" name="alAddress" type="text" value="${alPep.insuredaddress }"></td>
				<td colspan="2">邮箱</td>
			...
			<tr>
				...
				<td colspan="2"><input id="inName" name="inName" type="text" value="${inPep.insuredname }"></td>
				<td colspan="2">邮箱</td>
			...
			<tr>
				...
				<td colspan="7"><input id="inAddress" name="inAddress" type="text" value="${inPep.insuredaddress }"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td colspan="2">
					<input id="inBusinessLicenseNo" name="inBusinessLicenseNo" type="text" value="${inPep.identitynumber }" style="width:50%;">
...
<!-- 					</div> -->
				...
				<td colspan="2">
					<input id="inSafetyLicenseNo" name="inSafetyLicenseNo" type="text" value="${inPep.insuredtype }" style="width:50%;">
<!-- 					<div class="maincont_mid_cont_right_bot" style="display:inline-block;"> -->
...
每次事故每人医疗费用赔偿限额为每人人身伤亡赔偿限额的30%；每次事故财产损失为每次事故及累计赔偿限额的50%；
				...
				<td colspan="1"><input <c:if test="${policyManagerItem.itemOne==1 }">checked="checked"</c:if> id="itemOne" name="itemOne" type="checkbox" value="0901002"></td>
...
财产损失免赔额为人民币500元</td>
			...
			<tr>
				...
				<td colspan="1"><input <c:if test="${policyManagerItem.itemTwo==1 }">checked="checked"</c:if> id="itemTwo" name="itemTwo" type="checkbox" value="0901003"></td>
			...
			<tr>
				...
				<td colspan="1"><input <c:if test="${policyManagerItem.itemThree==1 }">checked="checked"</c:if> id="itemThree" name="itemThree" type="checkbox" value="0901004"></td>
			...
			<tr>
				...
				<td colspan="1"><input <c:if test="${policyManagerItem.itemFour==1 }">checked="checked"</c:if> id="itemFour" name="itemFour" type="checkbox" value="0901005"></td>
			...
			<tr>
				...
				<td colspan="6">自<input type="text" style="width:120px;height: 30px;" name="startDate" id="startDate" onchange="dateCheck();goEndDate();" onclick="WdatePicker({dateFmt:'yyyy/MM/dd'});" value="<fmt:formatDate value='${sessionScope.resultMap.STARTDATE }' pattern='yyyy/MM/dd'/>" 
				class="Wdate">起 至<input type="text" style="width:120px;height: 30px;" name="endDate" id="endDate" onchange="dateCheck()" onclick="WdatePicker({dateFmt:'yyyy/MM/dd'});" value="<fmt:formatDate value='${sessionScope.resultMap.ENDDATE }' pattern='yyyy/MM/dd'/>" 
		...
		<div class="maincont_mid_cont_right_bot" style="width: 100%;">
			<a style="cursor:pointer; display: block;margin: auto;width: 108px;height: 22px;padding: 5px 0px;float: none;" onclick="goZus()">确认修改</a>
		</div>
		</br>
		</br>
	 </div>
...
// 		    }
			...
			success : function(data) {
				if(data!=1){
					...
					fom.action = "${basePath}/insurePolicy/resubmitAfterUpdateForm";
		...
		$.ajax({
			type : "post",
			url : "${basePath}/codeControl/getGgCodeByObj?codeType=IndustryType&remark="
					+ parentVal,
			data : {},
			dataType : "json",
			contentType : "application/json;charset=UTF-8",
