...
public class SubmitInsureAfterUtil {
	...
	@Autowired
	private GuPolicyMainMapper mainMapper;
	...
	private InsuranceSingleRequest tidyInsureReq(String businessnos) {
		...
		if ("4".equals(user.getUserInd())) {// 如果是保险公司业务
		...
		for (int i = 0; i < guPolicyItemKind.size(); i++) {
			...
			GuPolicyItemKind itemKind = guPolicyItemKind.get(i);
			itemKindEhm.setKindCode(itemKind.getKindcode());// 险别代码
			itemKindEhm.setKindName(itemKind.getKindname());// 险别名称
			itemKindEhm.setAmount(String.valueOf(itemKind.getAmount()));// 保额
			itemKindEhm.setPremium(String.valueOf(itemKind.getActualpremium()));// 保费
			itemKindEhm.setRate(String.valueOf(itemKind.getRate()));// 费率
			itemKindEhm.setDeductible(itemKind.getRemark());// 免赔额
			itemKindEhm.setEachDeductible(itemKind.getRemark());//每次事故免赔额
			itemKindEhm.setEachtimelimit(String.valueOf(itemKind.getAmount()));// 每次事故责任限额
			if ("0901002".equals(guPolicyItemKind.get(i).getKindcode())) {
				...
				itemKindEhm.setEchLimitAmount(String.valueOf(itemKind
						.getUnitamount()));// 每人责任限额
			}
			itemKindEhm.setSumAmountlimit(String.valueOf(itemKind.getAmount()));// 累计赔偿限额
