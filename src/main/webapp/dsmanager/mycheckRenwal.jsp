  ...
  <div class="maincont_mid_cont_right">
	 <div class="tab-top-div">
		...
		<c:if test="${sessionScope.resultMap.POLICYNO != null }">
			...
			<h4 class="title-h1" style="color: red;">审核意见:${guPolicyMain.underDirections }</h4>
		<table class="nav-tab">
			<tr>
			...
			<tr>
				...
				<td colspan="2">
					<select style="width: 160px;"
						...
						<c:forEach items="${list_province}" var="cl" varStatus="st">
									<c:if test="${guPolicyMain.province == cl.codeCode}">
					...
					<select style="width: 160px;"
						...
						onchange="modifyArea('city','county','County')">
							<c:forEach items="${list_city}" var="ci" varStatus="st">
								<c:if test="${guPolicyMain.city == ci.codeCode}">
									<option selected="selected" value="${ci.codeCode}">${ci.codeCName }</option>
								</c:if>
								<c:if test="${guPolicyMain.city != ci.codeCode}">
					...
					<select style="width: 160px;"
						name="county" id="county">
							<c:forEach items="${list_county}" var="co" varStatus="st">
								<c:if test="${guPolicyMain.county == co.codeCode}">
									<option selected="selected" value="${co.codeCode}">${co.codeCName }</option>
								</c:if>
								<c:if test="${guPolicyMain.county != co.codeCode}">
			...
			<tr>
				...
				<td colspan="2">
					<input readonly="readonly" id="pepSum" name="pepSum" type="text" value="${emSize}" style="width:50%;">
				...
				<td colspan="2">
					...
					<div class="maincont_mid_cont_right_bot" style="display:inline-block;">
						<a style="cursor:pointer;" href="${standardLevelImg }">查看</a>
					</div>
			...
			<tr>
				...
				<td colspan="4">
					<select id="amount" name="amount" onchange="changeSumAmount()">
                        <option value="${unitAmount }"
							selected="selected">${unitAmount/10000
							}万</option>
						<c:if test="${unitAmount != 300000 }">
							<option value="300000">30万</option>
						</c:if>
						<c:if test="${unitAmount != 400000 }">
							<option value="400000">40万</option>
						</c:if>
						<c:if test="${unitAmount != 500000 }">
							<option value="500000">50万</option>
						</c:if>
						<c:if test="${unitAmount != 600000 }">
							<option value="600000">60万</option>
						</c:if>
						<c:if test="${unitAmount != 700000 }">
							<option value="700000">70万</option>
						</c:if>
						<c:if test="${unitAmount != 800000 }">
							<option value="800000">80万</option>
						</c:if>
						<c:if test="${unitAmount != 900000 }">
							<option value="900000">90万</option>
						</c:if>
						<c:if test="${unitAmount != 1000000 }">
			...
			<tr>
				...
				<td colspan="4"><input readonly="readonly" id="sumAmount" name="sumAmount" type="text" value="${guPolicyMain.sumAmount}"></td>
			</tr>
			<tr>
				<td colspan="3">年度累计赔偿限额</td>
				<td colspan="4"><input readonly="readonly" id="sumAmountTEMP" name="sumAmountTEMP" type="text" value="${guPolicyMain.sumAmount}"></td>
			</tr>
			<tr>
...
财产损失免赔额为人民币500元</td>
			...
			<tr>
				...
				<td colspan="6">自<input type="text" style="width:120px;height: 30px;" name="startDate" id="startDate" onchange="dateCheck();goEndDate();" onclick="WdatePicker({dateFmt:'yyyy/MM/dd'});" value="<fmt:formatDate value='${guPolicyMain.startDate}' pattern='yyyy/MM/dd'/>" 
				class="Wdate">起 至<input type="text" style="width:120px;height: 30px;" name="endDate" id="endDate" onchange="dateCheck()" onclick="WdatePicker({dateFmt:'yyyy/MM/dd'});" value="<fmt:formatDate value='${guPolicyMain.endDate}' pattern='yyyy/MM/dd'/>" 
				...
				<input hidden="hidden" id="starttime" name="starttime" value="<fmt:formatDate value='${sessionScope.resultMap.ENDDATE }' pattern='yyyy/MM/dd' />"/>
			</tr>
			<tr>
 ...
 <div id="showForm" style="dborder:1px solid red;height:3000px;background:#333;opacity:0.8;position:absolute;width:100%;top:0;display: none;">
	...
	function goZus() {
			...
			if (dateFlag2 < 0) {
				alert("起保日期不能小于上年终保日期!");
				return;
			}
			var nowDate = getFormatDate();
			var dateFlag1 = getDateToDate(nowDate,startDate);
			if(dateFlag1<0){
				alert("起保日期不能小于今天!");
				return;
			}
			...
			success : function(data) {
				if(data!=1){
					...
					fom.action = "${basePath}/dsmanager/getPlaceValue";
		...
		function goPep(){
			...
			var url = "${basePath}/dsmanager/getMySon?gobibi="+tempValue;
