...
function findArea(upperId, localId, currentLevel, o, lowerId) {
		...
		var areaLevle = document.getElementById("comLevel").value;//用户级别0:全国 1:省级 2:市级 3:县区级
		var areaCode = "";
		var upperCode = "";
		if (currentLevel <= areaLevle) {//currentLevel是当前是哪个select框 1:省的框 2:市的框 3:县区的框
			return;
		} else {
...
/**
 * 根据本级地区写入下级地区的代码
 * @param {} localId:本级select的id
 * @param {} lowerId:下级select的id
 * @param {} lowerCodeType:下级的类型 City:市 County:区县
 */
function getProvince(localId,lowerId,lowerCodeType) {
	var localIdVal = $j("#hidden_" + localId).val();
	
	$j.ajax({
		type:"get",
		url:"/codeControl/getProvince?codeType=" + lowerCodeType +"&remark=" + localIdVal,
		dataType:"json",
		contentType:"application/json;charset=utf-8",
		success:function(data){
			var ul = $j("#options_" + lowerId);//获取下级的ul
			$j.each(ul.children(),function(i,n) {//删除ul里所有的li
				n.remove();
			});
			
			/*<li class="open_selected" id="selected_province1" style="cursor: pointer;">选择省份</li>
			<li class="open" style="cursor: pointer;">新疆维吾尔自治区</li>*/
			$j.each(data,function(i,n) {//i是下标,n是其中一条对象
				ul.append("<li class=\"open\" style=\"cursor: pointer;\">"+n.codeCName+"</li>")
				.click(function(data) {
					document.getElementById("select_info_" + lowerId).innerHTML = this.innerHTML;
					var indexs = $j(this).index();
					$j("#" + lowerId).get(0).selectedIndex = indexs; //设置Select索引值为1的项选中 
					$j("#hidden_" + lowerId).val($j("#" + lowerId).val());
				})
				/*.html(n.codeCName)
				.addClass("open")
				.mouseover(function() {
					$j(this).css({color:"#fff",backgroundColor :"#1ec2ff"});
					this.style.color = "#fff";
					this.style.backgroundColor = "#1ec2ff";
				})
				.mouseout(function() {
					$j(this).css({color:"#79A2BD",backgroundColor :""});
					this.style.color = "#79A2BD";
					this.style.backgroundColor = "";
				})
				.click(function() {                                                                                                             
