...
<title>My JSP 'addInveStigateExpertList.jsp' starting page</title>
		...
		<form action="${basePath }/organ/yuyueListContinue" id="fm" name="fm"
			...
			<table cellpadding="0" cellspacing="0" width="100%" border="0"
				...
				<c:forEach items="${sessionScope.pagination.resultList }"
					var="govExpert" varStatus="listSize">
					...
					<!-- 名称 -->
					<td>${govExpert.EXNAME }</td>
					<!-- 地址 -->
					<td><tlds:name codeCode='${govExpert.PROVINCE }'
							codeType='Province'></tlds:name>- <tlds:name
							codeCode='${govExpert.CITY }' codeType='City'></tlds:name>- <tlds:name
							codeCode='${govExpert.COUNTY }' codeType='County'></tlds:name>-
						${govExpert.ADDRESS }</td>
					<!-- 评估范围 -->
					<td><tlds:name codeType='IndustryCategories'
							codeCode='${govExpert.BUSINESSCLASS }'></tlds:name>
					</td>
					<!-- 联系人 -->
					<td>${govExpert.EXNAME }</td>
					<!-- 联系方式 -->
					<td>${govExpert.PHONE }</td>
					<!-- 操作 -->
					<td style="width:200px;">
						<!-- 此处判断：如果预约表中有当前的这个机构的信息，则显示“已预约” --> <c:if
							test="${govExpert.EXPCODE == govExpert.SUBORGCODE }">
							<div class="select_list" style="margin-left:32px;">
								<a
									href="${basePath }/subScribe/sendGovOrganByOrgCode?expCode=${govExpert.EXPCODE }"
									style="display:inline-block;width:80px;height:27px;-moz-border-radius: 0px;
							-webkit-border-radius: 0px;border-radius:0px;"
									target="mainFrame">已预约</a>
							</div>
						</c:if> <c:if test="${govExpert.EXPCODE != govExpert.SUBORGCODE }">
							<div class="select_list" style="margin-left:32px;">
								<a
									href="${basePath }/subScribe/sendGovOrganByOrgCode?expCode=${govExpert.EXPCODE }"
									style="display:inline-block;width:80px;height:27px;-moz-border-radius: 0px;
