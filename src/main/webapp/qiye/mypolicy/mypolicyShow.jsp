...
<head>
<base href="<%=basePath%>">
<script type="text/javascript" src="${basePath }/js/selectcss.js"></script>
...
<!-- 			</c:if> -->
		...
		<table class="nav-tab">
			...
			<tr>
				...
				<td colspan="2">${alPep.identitynumber }</td>
				<td colspan="2">安全生产许可证号</td>
				<td colspan="2">${TempSafetyLicenseNo }</td>
			</tr>
			<tr>
...
财产损失免赔额为人民币500元</td>
			...
			<tr>
				...
				<td colspan="6">自<fmt:formatDate value='${sessionScope.resultMap.STARTDATE }' pattern='yyyy年MM月dd'/>日00:00:00时起 至<fmt:formatDate value='${sessionScope.resultMap.ENDDATE }' pattern='yyyy年MM月dd'/>日23:59:59止(以最终签发的保险单为准)为保证保险合同在拟定的保险起期生效，请您在保险起期前5个工作日前办理投保手续、提交投保资料并支付保费。如您未能按时办理投保手续或保费未能按时到账，导致保险合同未在拟定的保险起期生效，发生保险事故，本公司不承担保险责任。</td>
			</tr>
			<tr>
			...
			<c:if test="${resultMap.SPECIALPROVISIONS!='' || resultMap.SPECIALPROVISIONS != 'null'  }">
				<tr>
					<td colspan="2">批改内容</td>
					<td style="color: red;"  colspan="5">${special.SPECIALPROVISIONS }</td>
					<td colspan="5" style="color:red;">${resultMap.SPECIALPROVISIONS }</td>
				</tr>
			</c:if>
		</table>
		<div style="text-align:left;width:100%;">
        				<span style="display:inline-block;width:100px;">争议解决方式：</span>
